...
**18 workstreams · 113 units of work · 63 not done.**
...
## Take the moves from the concepts — next
...
- **TK-12 · Build the rest of the next store, and wire the flow** — `queued` — Two pages of the next store exist: the homepage and the product page. The cart, the checkout, the post-purchase page, the policy picker and the comparison are waiting on the next design round, and the buying flow is not wired at all.That is the order it was asked for. When the next set of designs lands, the component layer and the data model are already there — assets/next.css and assets/next.js were written for the whole flow rather than for two pages.
